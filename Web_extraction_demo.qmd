...
fandango_data <- tibble(Fandango_Stars = seq(0,5, .5)) %>% 
...
rt_data <- tibble(RT_norm_round = seq(0,5, .5)) %>% 
  full_join(rt_count) %>% 
  mutate(count = case_when(
    is.na(n) ~ 0,
    !is.na(n) ~ as.numeric(n)
...
rt_user_data <- tibble(RT_user_norm_round = seq(0,5, .5)) %>% 
...
mc_data <- tibble(Metacritic_norm_round = seq(0,5, .5)) %>% 
  full_join(mc_count) %>% 
  mutate(count = case_when(
    is.na(n) ~ 0,
    !is.na(n) ~ as.numeric(n)
...
mc_user_data <- tibble(Metacritic_user_norm_round = seq(0,5, .5)) %>% 
...
imdb_user_tibble <- tibble(IMDB_norm_round = seq(0,5, .5)) %>% 
